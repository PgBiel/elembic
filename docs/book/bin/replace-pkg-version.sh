...
jq .[1] | jq '.sections[] |= (if .Chapter? == null then . else (.Chapter.content, .Chapter.sub_items[].Chapter.content) |= gsub("@preview/elembic:X.X.X"; "@preview/elembic:'$PKG_VERSION'") end)'

...
#assert.eq(cast(none, native.content_), (true, []))
#assert.eq(cast(none, types.union(native.content_, native.ratio_)), (true, []))
#assert.eq(cast(123, native.content_), (false, "expected none, content, string or symbol, found integer"))

#assert.eq(cast([abc], content), (true, [abc]))
#assert.eq(cast("abc", content), (true, [abc]))
#assert.eq(cast(sym.eq, content), (true, [#sym.eq]))
#assert.eq(cast([= efg], content), (true, [= efg]))
#assert.eq(cast(123, content), (false, "expected none, content, string or symbol, found integer"))

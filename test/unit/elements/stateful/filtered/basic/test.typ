...
#show: e.filtered(wibble.with(number: 10), e.stateful.set_(wobble, color: orange))
#show: e.filtered(wibble.with(number: 20), e.stateful.apply(e.set_(wobble, color: purple), e.set_(wobble, inner: [twenty])))
...
#wibble(
  number: 20,
  run: it => wobble(run: it => {
    assert.eq(it.color, purple)
    assert.eq(it.inner, [twenty])
  })
)
